...
void ErrorFinderManager::performConsolidation(int argc,char *argv[])
{
	...
	*/
	...
	if ((SITUATION_NO == 1)	||	(SITUATION_NO == 3))	//findTruePctErrors //consolidator.findTruePctErrors( eCalculator, MA_SNP_END, true, WINDOW,MA_THRESHOLD, EMPIRICAL_MA_RESULT);
		{
			if(T_TRIM )
				{
							...
							for (int l=0;l<consolidator.m_matches[i][j].size();l++ ) //for(int l=0;l<consolidator.m_trueMatches[i][j].size();l++)
								{
									...
									if (!ifcontd)
										{
											int t1_ = consolidator.m_trueMatches[ i ][ j ][ l ].start + ( consolidator.m_trueMatches[ i ][ j ][ l ].end -consolidator.m_trueMatches[ i ][ j ][ l ].start ) * 0.25;
											int t2_ = consolidator.m_trueMatches[ i ][ j ][ l ].end -( consolidator.m_trueMatches[ i ][ j ][ l ].end -consolidator.m_trueMatches[ i ][ j ][ l ].start ) * 0.25;
											...
											int temp1_ = consolidator.m_trueMatches[ i ][ j ][ l ].start +( consolidator.m_trueMatches[ i ][ j ][ l ].end -consolidator.m_trueMatches[ i ][ j ][ l ].start ) * 0.15; //Should probably stop doing this
											int temp2_ = consolidator.m_trueMatches[ i ][ j ][ l ].end -( consolidator.m_trueMatches[ i ][ j ][ l ].end -consolidator.m_trueMatches[ i ][ j ][ l ].start ) * 0.15;
											...
											int len = consolidator.m_matches[ randPers1 ][ randPers2 ][ pos ].end - consolidator.m_matches[ randPers1 ][ randPers2 ][ pos ].start;
											if( len >= fend || len <= 0)
												{
													continue;
												}
			...
			if( option.compare("weightedOutput") == 0 )
				{
					...
					if(consolidator.isUserSuppliedWeights())
						{ //the user has supplied their own weights.
	...
	else	// for (	(situation_no == 2)	&&	(situation_no == 4)		) //consolidator.findTrueSimplePctErrors( eCalculator, PIELENGTH, false, WINDOW, MA_THRESHOLD, EMPIRICAL_MA_RESULT );
		{
			if (T_TRIM)
				{
					std::cerr<<"entering into HoldOut Mode Matching"<<std::endl;
